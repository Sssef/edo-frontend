$(function() {
    ...
    if (isChrome()) {
        $('<script />', { type : 'text/javascript', src : '/js/sks/chrome.js'}).appendTo('head');
    }
    ...
    $d.on('click', '#docsBindForm .pages a', function (e) {
        e.preventDefault();
        const $this = $(this);
        const page = $this.attr('data-page');

        refreshLinkDocsList(page);
    });

    $d.on('change', '#docFilterType, #docFilterDirection', function (e) {
        e.preventDefault();

        refreshLinkDocsList();
    });

    function refreshLinkDocsList(page) {
        const $docLinkModal = $('#docLinkModal');
        const docId = $docLinkModal.attr('data-docId');

        const data = {
            page: page ? page : $("#docsBindForm .pages .active").attr("data-page"),
            documentType: $('#docFilterType').val(),
            direction: $('#docFilterDirection').val(),
            frag: true,
            excludeDocId: docId
        };
    ...
    $d.on('click', '#doc-sign', function(e) {
        const $this = $(this);
        const id = $this.closest('.document').data('id');

        if (!isChrome()) {
            alert('Подпись документов доступна только в браузере Google Chrome!');
            return;
        } else if (!window.sks) {
            alert('Компонент безопасности не был проиницилизирован. ' +
                'Попробуйте отключить блокировщик рекламы и попробуйте ещё раз.');
            return;
        } else {
            window.sks.setProvider(8)
                .then(function(){
                    getXml(id);
                })
                .catch(function (e) {
                    if (e.code === 14) {
                        alert('Ошибка при обращении к компоненту безопасности. ' +
                            '1. Проверьте, что у вас установлено приложение Faktura.ru\n' +
                            '2. Проверьте, что у вас установлено и включено расширение Faktura.ru');
                    } else {
                        const errMsg = e.message;
                        alert('Ошибка при обращении к ключу.' + (errMsg ? ' Подробности: ' + errMsg : ''));
                    }
                });
        }

        function getXml(id) {
            ajax('GET', '/docs/' + id + '/xml', {}, 'json', function (response) {
                if (response && response.xml) {
                     signXml(id, response.xml);
                } else {
                    alert('Ошибка получения xml представления документа');
                }
            }, function (xhr) {
        ...
        function signXml(docId, xml) {
            let signedXml;

            window.sks.importKey(1)
                .then(function () {
                    return window.sks.createSignature(xml);
                })
                .then(function (result) {
                    console.log(result);
                    signedXml = result;

                    return window.sks.getCert(1);
                })
                .then(function (cert) {
                    return window.sks.getCertInfo(cert);
                })
                .then(function (certInfo) {
                  const cert = certInfo.cert;
                  console.log(cert);

                  send(docId, xml, signedXml, cert);
                })
                .catch(function (error) {
                    const errMsg = error.message;
                    alert('Ошибка подписи документа' + (errMsg ? '. Подробности: ' + errMsg : ''));
                });
        }

        function send(docId, xml, signedXml, cert) {
            const data = {
                xml: xml,
                signedXml: signedXml,
                certificate: cert
            ...
            ajax('POST', '/docs/' + docId + "/sign", data, 'json', function (response) {
                alert(response.result);

                location.reload();
            }, function(xhr) {
                errorMessages(xhr, $('body'));
            });
        }
    });
...
function init() {
    ...
    initGroup($('input[data-dadata-type]'), function (e) {
        e.suggestions({
            token: "9aec83d330fd615c35929fa64569b1473ccd16ba",
